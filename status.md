...
**同日 P0-6 亦已完成**：`scripts/build_d1_pdf.py` 增加离线 Mermaid 渲染（graph/sequence 两类）、对缺失 pagebreak 容错、支持 fenced code，并保留 `[DIAGRAM:x]` 通路；`SOURCE` 切到 v0.2。独立复核：`py_compile` PASS，v0.2 构建为 **13 页**，v1 稿仍构建为 14 页未回归，验证使用临时输出路径故 `output/pdf/` 兜底未被覆盖。**实测 13 页远低于原预估 22–26 页，页数余量 17 页**，P0-1..P0-5 无需为省页数压缩内容。风险 R1 关闭。

据此当前口径为：① v1 稿不再作构建源，仅作历史参考与文本来源；② 数字冻结日锁定 8/9，在此之前 D1 与字幕中 OAR N=10 数字不写定值；③ D1 v0.2 源稿已补入 P0-1 Gate1 分层指标、P0-2 指标体系与预期效果、P0-3 算法设计；④ 下一动作为 **P0-4 OpenClaw** 与 **P0-5 横向对比表**；⑤ 正式 `output/pdf/` 待内容完成后（8/10–8/23）用 v0.2 一次性重建，不产生中间版本。
...
| D1 技术方案 | **BASE-FROZEN / BUILDABLE / CONTENT-OPEN** | 底稿已定为 v0.2 审阅稿（DEC-1），v1 稿转为历史参考不再作构建源；**P0-6 已完成**：`scripts/build_d1_pdf.py` 支持离线 Mermaid 渲染并容错缺失 pagebreak，v0.2 实测 **13 页**、v1 兼容 14 页、`py_compile` PASS，**页数余量 17 页**；P0-1/P0-2/P0-3 已补入 v0.2 Markdown 源稿；P0-4 OpenClaw 与 P0-5 横向对比表仍待补；正式 `output/pdf/` 尚未用 v0.2 重建（安排在内容完成后的 8/10–8/23），现有 14 页 PDF 保留为兜底 |
...
| 文档一致性 | REVIEW-OPEN / ATTACK-PROOF-SYNCED | D1 v0.2 已同步 2026-07-26 clean attack proof，并已补入 Gate1 分层指标、五维指标体系与算法设计；P0-4/P0-5、OAR N=10 数字冻结、D3 重排与正式 PDF 重建仍待完成 |
...
| 周 | 任务 | 冻结点 |
|---|---|---|
| ~~7/26~~ | ~~五项决策定案~~ + ~~P0-6 修构建管线~~ | ✅ **决策已冻结** + ✅ **P0-6 完成**（v0.2 = 13 页） |
| 7/27–8/2 | P0-1..P0-5 起草（P0-1..P0-3 已入源稿；页数余量 17 页，可放开写） | — |
...
| ~~P0-6~~ | ~~修 PDF 构建管线使其能构建 v0.2~~ → ✅ **DONE 2026-07-26**：v0.2 = 13 页、v1 = 14 页未回归、余量 17 页 | 全部 |
...
1. **P0-4 / P0-5（下一动作）**：补"兼容 OpenClaw 类智能体"与横向对比表；对比表只用事实源已核实条目。P0-1 Gate1 分层指标、P0-2 指标体系、P0-3 算法设计已进入 v0.2 源稿，仍需最终通读压口径。
2. 内容完成后（8/10–8/23）用 v0.2 重建正式 `output/pdf/` 并记录最终页数与 SHA-256；在此之前保留现有 14 页兜底，不产生中间版本。
...
- 2026-07-26 D1 收口仍为文档工作：P0-1/P0-2/P0-3 已补入 D1 v0.2 Markdown 源稿；未重跑产品、故障、kind、性能或攻击实验，未修改产品/测试代码，未替换当前 14 页 PDF。
